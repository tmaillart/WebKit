 ...
 * @user_data: user data
 *
 * Function invoked at promise initialization
 *
 * It is called like a js function, so exceptions raised will not be propagated
 * to the context, but handled by the promise causing a rejection.
 * @resolve and @reject can be reffed for later use to handle async tasks completion.
 *
 * Since: 2.48
 */
 ...
 * @context: a #JSCContext
 * @executor: (scope call) (nullable) (closure user_data): an initialization callback
 * @user_data: (nullable): user data passed in @executor
 *
 * Create a new Promise, executor will be invoked during promise initialization
